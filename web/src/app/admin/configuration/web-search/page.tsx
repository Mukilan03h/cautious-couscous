...
type WebContentProviderType = "firecrawl" | "esa_web_crawler" | (string & {});
...
const CONTENT_PROVIDER_LABEL: Record<string, string> = {
  firecrawl: "Firecrawl",
  esa_web_crawler: "ESA Web Crawler",
};
...
> = {
  ...
  esa_web_crawler: {
    ...
    description:
      "ESA’s built-in crawler processes URLs returned by your search engine.",
  },
};
...
export default function Page() {
  ...
  const displayContentProviders = useMemo(() => {
    ...
    const hasESA = providers.some(
      (provider) => provider.provider_type === "esa_web_crawler"
    );
    ...
    if (!hasESA) {
      providers.unshift({
        id: -1,
        name: "ESA Web Crawler",
        provider_type: "esa_web_crawler",
        is_active: true,
        config: null,
        has_api_key: true,
      });
  ...
  const currentContentProviderType = useMemo(() => {
    const nonDefaultActive = contentProviders.find(
      (provider) =>
        provider.is_active && provider.provider_type !== "esa_web_crawler"
    );
  ...
  const handleActivateContentProvider = async (
    ...
    try {
      if (provider.provider_type === "esa_web_crawler") {
        const response = await fetch(
  ...
  const handleDeactivateContentProvider = async (
    ...
    try {
      // For esa_web_crawler (virtual provider with id -1), use reset-default
      // For real providers, use the deactivate endpoint
      const endpoint =
        providerType === "esa_web_crawler" || providerId < 0
          ? "/api/admin/web-search/content-providers/reset-default"
          ...
          <div className="flex flex-col gap-3 self-stretch">
            ...
            <div className="flex flex-col gap-2 self-stretch">
              {displayContentProviders.map((provider) => {
                ...
                const providerId = provider.id;
                const hasStoredKey =
                  provider.provider_type === "esa_web_crawler"
                    ? true
