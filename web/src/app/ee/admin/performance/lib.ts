...
import { ESABotAnalytics, QueryAnalytics, UserAnalytics } from "./usage/types";
...
export const useESABotAnalytics = (timeRange: DateRangePickerValue) => {
  const url = buildApiPath("/api/analytics/admin/esabot", {
    start: convertDateToStartOfDay(timeRange.from)?.toISOString(),
    end: convertDateToEndOfDay(timeRange.to)?.toISOString(),
  });
  const swrResponse = useSWR<ESABotAnalytics[]>(url, errorHandlingFetcher); // TODO

  return {
    ...swrResponse,
    refreshESABotAnalytics: () => mutate(url),
  };
};
