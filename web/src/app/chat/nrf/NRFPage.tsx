...
export default function NRFPage({
  ...
  // If user toggles the "Use ESA" switch to off, prompt a modal
  const handleUseESAToggle = (checked: boolean) => {
    if (!checked) {
      setShowTurnOffModal(true);
    } else {
      setUseESAAsNewTab(true);
        ...
        {({ getRootProps }) => (
          ...
          >
            <div className="pointer-events-auto absolute top-[40%] left-1/2 -translate-x-1/2 -translate-y-1/2 text-center w-[90%] lg:max-w-3xl">
              ...
              >
                {isNight
                  ? "End your day with ESA"
                  : "Start your day with ESA"}
              </h1>
