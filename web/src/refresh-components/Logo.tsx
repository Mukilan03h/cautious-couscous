...
import { ESAIcon, ESALogoTypeIcon } from "@/components/icons/icons";
...
export default function Logo({ folded, className }: LogoProps) {
  ...
  return settings.enterpriseSettings?.application_name ? (
    ...
    <ESAIcon size={FOLDED_SIZE} className={cn("flex-shrink-0", className)} />
  ) : (
    <ESALogoTypeIcon size={UNFOLDED_SIZE} className={className} />
  );
}
