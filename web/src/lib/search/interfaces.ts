...
export interface MinimalESADocument {
  document_id: string;
  semantic_identifier: string | null;
}

export interface ESADocument extends MinimalESADocument {
  link: string;
  source_type: ValidSources;
  blurb: string;
  boost: number;
...
export interface LoadedESADocument extends ESADocument {
  icon: React.FC<{ size?: number; className?: string }>;
}

export interface SearchESADocument extends ESADocument {
  is_relevant: boolean;
  relevance_explanation: string;
}

export interface FilteredESADocument extends ESADocument {
  included: boolean;
}
export interface DocumentInfoPacket {
  top_documents: ESADocument[];
...
export interface SearchResponse {
  ...
  answer: string | null;
  quotes: Quote[] | null;
  documents: SearchESADocument[] | null;
...
export interface SearchRequestArgs {
  ...
  updateDocs: (documents: ESADocument[]) => void;
