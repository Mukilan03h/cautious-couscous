...
export default function CreateCredential({
  ...
  const handleSubmit = async (
    ...
    console.log("[CreateCredential] Validation errors:", errors);
    if (Object.keys(errors).length > 0) {
      console.log("[CreateCredential] Form has validation errors, not submitting");
    ...
    console.log("[CreateCredential] Form values:", values);
    console.log("[CreateCredential] Extracted credentialValues:", credentialValues);
    ...
    console.log("[CreateCredential] Filtered credentialValues (sent to backend):", filteredCredentialValues);
      ...
      initialValues={
        {
          ...
          // Initialize logic for dynamic fields to avoid uncontrolled input warning
          ...Object.keys(credentialTemplate).reduce((acc, key) => {
            if (key !== "authMethods" && key !== "authentication_method") {
              acc[key] = "";
            }
            return acc;
          }, {} as any),
          ...(templateWithAuth?.authMethods?.reduce((acc: any, method: any) => {
            Object.keys(method.fields).forEach((key: string) => {
              acc[key] = "";
            });
            return acc;
          }, {}) || {}),
