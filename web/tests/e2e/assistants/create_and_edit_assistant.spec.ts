...
import { ESAApiClient } from "../utils/esaApiClient";
  ...
  test.describe("With Knowledge", () => {
    ...
    test.afterAll(async ({ browser }: { browser: Browser }) => {
      ...
      if (ccPairId && documentSetId) {
        ...
        const cleanupClient = new ESAApiClient(page);
    ...
    test("should create and edit assistant with Knowledge enabled", async ({
      ...
      const esaApiClient = new ESAApiClient(page);
      ccPairId = await esaApiClient.createFileConnector("Test Connector");
      documentSetId = await esaApiClient.createDocumentSet(
        "Test Document Set",
        [ccPairId]
      );
