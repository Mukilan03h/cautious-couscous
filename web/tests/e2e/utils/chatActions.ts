...
export async function verifyDefaultAssistantIsChosen(page: Page) {
  await expect(page.getByTestId("esa-logo")).toBeVisible({ timeout: 5000 });
}
...
export async function sendMessage(page: Page, message: string) {
  await page.locator("#esa-chat-input-textarea").click();
  await page.locator("#esa-chat-input-textarea").fill(message);
  await page.locator("#esa-chat-input-send-button").click();
  await page.waitForSelector('[data-testid="esa-ai-message"]');
