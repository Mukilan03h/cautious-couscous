...
/**
 * API Client for ESA backend operations in E2E tests.
 *
 * Provides a type-safe, abstracted interface for interacting with the ESA backend API.
 ...
 * **Usage Example:**
 * ```typescript
 * const client = new ESAApiClient(page);
...
export class ESAApiClient {
  ...
  private async handleResponseSoft(
    ...
    if (!response.ok()) {
      const errorText = await response.text();
      console.error(
        `[ESAApiClient] ${errorMessage}: ${response.status()} - ${errorText}`
      );
      return false;
    }
    return true;
   ...
   *
   * @param message - The message to log (will be prefixed with "[ESAApiClient]")
   */
  private log(message: string): void {
    console.log(`[ESAApiClient] ${message}`);
  }
