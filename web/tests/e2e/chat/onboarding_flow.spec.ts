...
test.describe("First user onboarding flow", () => {
  test("completes onboarding wizard and unlocks chat input", async ({
    ...
    const dismissNewTeamModal = async () => {
      ...
      const tryESAButton = page
        .getByRole("button", { name: /Try ESA while waiting/i })
        .first();
      if ((await tryESAButton.count()) > 0) {
        await tryESAButton.click();
        return true;
      }
      return false;
    };
    ...
    await expect(page.getByText("What should ESA call you?")).toBeVisible();
    ...
    const chatInput = page.locator("#esa-chat-input-textarea");
