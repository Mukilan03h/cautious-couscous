...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --delete-data)
            ...
            ;;
        --help|-h)
            echo "ESA Installation Script"
            echo ""
            echo "Usage: $0 [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --shutdown     Stop (pause) ESA containers"
            echo "  --delete-data  Remove all ESA data (containers, volumes, and files)"
            echo "  --help, -h     Show this help message"
            echo ""
            echo "Examples:"
            echo "  $0                    # Install ESA"
            echo "  $0 --shutdown         # Pause ESA services"
            echo "  $0 --delete-data      # Completely remove ESA and all data"
            exit 0
            ;;
        *)
...
if [ "$SHUTDOWN_MODE" = true ]; then
    echo ""
    echo -e "${BLUE}${BOLD}=== Shutting down ESA ===${NC}"
    echo ""
    
    if [ -d "esa_data/deployment" ]; then
        print_info "Stopping ESA containers..."

        # Check if docker-compose.yml exists
        if [ -f "esa_data/deployment/docker-compose.yml" ]; then
            ...
            (cd esa_data/deployment && $COMPOSE_CMD -f docker-compose.yml stop)
            if [ $? -eq 0 ]; then
                print_success "ESA containers stopped (paused)"
            else
                print_error "Failed to stop containers"
                exit 1
            fi
        else
            print_warning "docker-compose.yml not found in esa_data/deployment"
        fi
    else
        print_warning "ESA data directory not found. Nothing to shutdown."
    fi
    
    echo ""
    print_success "ESA shutdown complete!"
    exit 0
fi
...
if [ "$DELETE_DATA_MODE" = true ]; then
    echo ""
    echo -e "${RED}${BOLD}=== WARNING: This will permanently delete all ESA data ===${NC}"
    echo ""
    print_warning "This action will remove:"
    echo "  • All ESA containers and volumes"
    ...
    print_info "Removing ESA containers and volumes..."
    ...
    if [ -d "esa_data/deployment" ]; then
        ...
        if [ -f "esa_data/deployment/docker-compose.yml" ]; then
            ...
            (cd esa_data/deployment && $COMPOSE_CMD -f docker-compose.yml down -v)
            if [ $? -eq 0 ]; then
                print_success "ESA containers and volumes removed"
            else
    ...
    if [ -d "esa_data" ]; then
        ...
        print_warning "No esa_data directory found"
    fi
    
    echo ""
    print_success "All ESA data has been permanently deleted!"
    exit 0
fi
...
echo "              |___/       "
echo -e "${NC}"
echo "Welcome to ESA Installation Script"
echo "===================================="
echo ""

# User acknowledgment section
echo -e "${YELLOW}${BOLD}This script will:${NC}"
echo "1. Download deployment files for ESA into a new 'esa_data' directory"
...
GITHUB_RAW_URL="https://raw.githubusercontent.com/esa-dot-app/esa/main/deployment/docker_compose"
...
if [ "$RESOURCE_WARNING" = true ]; then
    echo ""
    print_warning "ESA recommends at least ${EXPECTED_DOCKER_RAM_GB}GB RAM and ${EXPECTED_DISK_GB}GB disk space for optimal performance."
    echo ""
...
if [ -d "esa_data" ]; then
    ...
    print_success "Using existing esa_data directory"
else
    mkdir -p esa_data/deployment
    mkdir -p esa_data/data/nginx/local
    print_success "Directory structure created"
fi

# Download all required files
print_step "Downloading ESA configuration files"
...
COMPOSE_FILE="esa_data/deployment/docker-compose.yml"
...
ENV_TEMPLATE="esa_data/deployment/env.template"
...
NGINX_BASE_URL="https://raw.githubusercontent.com/esa-dot-app/esa/main/deployment/data/nginx"

# Download app.conf.template
NGINX_CONFIG="esa_data/data/nginx/app.conf.template"
...
NGINX_RUN_SCRIPT="esa_data/data/nginx/run-nginx.sh"
...
README_FILE="esa_data/README.md"
...
touch "esa_data/data/nginx/local/.gitkeep"
print_success "All configuration files downloaded successfully"

# Set up deployment configuration
print_step "Setting up deployment configs"
ENV_FILE="esa_data/deployment/.env"

# Check if services are already running
if [ -d "esa_data/deployment" ] && [ -f "esa_data/deployment/docker-compose.yml" ]; then
    # Determine compose command
    if docker compose version &> /dev/null; then
        COMPOSE_CMD="docker compose"
    elif command -v docker-compose &> /dev/null; then
    ...
    if [ -n "$COMPOSE_CMD" ]; then
        ...
        RUNNING_CONTAINERS=$(cd esa_data/deployment && $COMPOSE_CMD -f docker-compose.yml ps -q 2>/dev/null | wc -l)
        if [ "$RUNNING_CONTAINERS" -gt 0 ]; then
            print_error "ESA services are currently running!"
            echo ""
            print_info "To make configuration changes, you must first shut down the services."
            echo ""
            print_info "Please run the following command to shut down ESA:"
...
(cd esa_data/deployment && $COMPOSE_CMD -f docker-compose.yml pull --quiet)
if [ $? -eq 0 ]; then
...
# Start services
print_step "Starting ESA services"
print_info "Launching containers..."
echo ""
if [ "$USE_LATEST" = true ]; then
    print_info "Force pulling latest images and recreating containers..."
    (cd esa_data/deployment && $COMPOSE_CMD -f docker-compose.yml up -d --pull always --force-recreate)
else
    (cd esa_data/deployment && $COMPOSE_CMD -f docker-compose.yml up -d)
fi
if [ $? -ne 0 ]; then
    print_error "Failed to start ESA services"
    exit 1
fi
...
CONTAINERS=$(cd esa_data/deployment && $COMPOSE_CMD -f docker-compose.yml ps -q 2>/dev/null)

for CONTAINER in $CONTAINERS; do
    CONTAINER_NAME=$(docker inspect --format '{{.Name}}' "$CONTAINER" | sed 's/^\/\|^esa_data_deployment_//g')
...
if [ "$RESTART_ISSUES" = true ]; then
    ...
    echo "  (cd esa_data/deployment && $COMPOSE_CMD -f docker-compose.yml logs)"
    echo ""
    print_info "If the issue persists, please contact: founders@esa.app"
...
check_esa_health() {
    ...
    print_info "Checking ESA service health..."
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        printf "\r\033[KChecking ESA service%s (%dm %ds elapsed)" "$dots" "$minutes" "$seconds"
...
# Run health check
if check_esa_health; then
    echo ""
    echo -e "${GREEN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${GREEN}${BOLD}   🎉 ESA service is ready! 🎉${NC}"
    echo -e "${GREEN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
else
    ...
    echo -e "${YELLOW}${BOLD}   ⚠️  ESA containers are running ⚠️${NC}"
    echo -e "${YELLOW}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
fi
echo ""
print_info "Access ESA at:"
...
echo ""
print_info "Refer to the README in the esa_data directory for more information."
echo ""
print_info "For help or issues, contact: founders@esa.app"
echo ""
