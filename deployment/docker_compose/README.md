...
To set up ESA there are several options, ESA supports the following for deployment:
...
**For more detailed guides, please refer to the documentation: https://docs.esa.app/deployment/overview**

## install.sh script

```
curl -fsSL https://raw.githubusercontent.com/esa-dot-app/esa/main/deployment/docker_compose/install.sh > install.sh && chmod +x install.sh && ./install.sh
```

This provides a guided installation of ESA via Docker Compose. It will deploy the latest version of ESA
and set up the volumes to ensure data is persisted across deployments or upgrades.

The script will create an esa_data directory, all necessary files for the deployment will be stored in
...
ESA maintains backwards compatibility across all minor versions following SemVer. If following the install.sh script (or through Docker Compose), you can
...
IMAGE_TAG is the version of ESA to run. It is recommended to leave it as latest to get all updates with each redeployment.
