...
STACK_NAMES=(
  "${ENVIRONMENT}-esa-nginx-service"
  "${ENVIRONMENT}-esa-web-server-service"
  "${ENVIRONMENT}-esa-backend-background-server-service"
  "${ENVIRONMENT}-esa-backend-api-server-service"
  "${ENVIRONMENT}-esa-model-server-inference-service"
  "${ENVIRONMENT}-esa-model-server-indexing-service"
  "${ENVIRONMENT}-esa-vespaengine-service"
  "${ENVIRONMENT}-esa-redis-service"
  "${ENVIRONMENT}-esa-postgres-service"
  "${ENVIRONMENT}-esa-cluster"
  "${ENVIRONMENT}-esa-acm"
  "${ENVIRONMENT}-esa-efs"
  )
...
delete_stack() {
  ...
  if [ "$stack_name" == "${ENVIRONMENT}-esa-cluster" ]; then
      echo "Removing all objects and directories from the esa config s3 bucket."
