...
if [ -n "$S3_BUCKET_FROM_CONFIG" ]; then
    ...
    S3_BUCKET="${S3_BUCKET:-esa-ecs-fargate-configs}"
fi

INFRA_ORDER=(
  "esa_efs_template.yaml"
  "esa_cluster_template.yaml"
  "esa_acm_template.yaml"
)

# Deployment order for services
SERVICE_ORDER=(
  "esa_postgres_service_template.yaml"
  "esa_redis_service_template.yaml"
  "esa_vespaengine_service_template.yaml"
  "esa_model_server_indexing_service_template.yaml"
  "esa_model_server_inference_service_template.yaml"
  "esa_backend_api_server_service_template.yaml"
  "esa_backend_background_server_service_template.yaml"
  "esa_web_server_service_template.yaml"
  "esa_nginx_service_template.yaml"
)
...
deploy_infra_stacks() {
    for template_name in "${INFRA_ORDER[@]}"; do
      ...
      if [[ "$template_name" == "esa_acm_template.yaml" ]]; then
...
echo "Starting deployment of ESA to ECS Fargate Cluster..."
deploy_infra_stacks
deploy_services_stacks
