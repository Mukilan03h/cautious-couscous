...
from esa.configs.app_configs import LOG_ONYX_MODEL_INTERACTIONS
from esa.configs.app_configs import MOCK_LLM_RESPONSE
from esa.configs.chat_configs import QA_TIMEOUT
from esa.configs.model_configs import (
    DISABLE_LITELLM_STREAMING,
)
from esa.configs.model_configs import GEN_AI_TEMPERATURE
from esa.configs.model_configs import LITELLM_EXTRA_BODY
from esa.llm.interfaces import LanguageModelInput
from esa.llm.interfaces import LLM
from esa.llm.interfaces import LLMConfig
from esa.llm.interfaces import STANDARD_TOOL_CHOICE_OPTIONS
from esa.llm.interfaces import ToolChoiceOptions
from esa.llm.llm_provider_options import AZURE_PROVIDER_NAME
from esa.llm.llm_provider_options import OLLAMA_PROVIDER_NAME
from esa.llm.llm_provider_options import VERTEX_CREDENTIALS_FILE_KWARG
from esa.llm.llm_provider_options import VERTEX_LOCATION_KWARG
from esa.llm.model_response import ModelResponse
from esa.llm.model_response import ModelResponseStream
from esa.llm.utils import is_true_openai_model
from esa.llm.utils import model_is_reasoning_model
from esa.server.utils import mask_string
from esa.utils.logger import setup_logger
from esa.utils.long_term_log import LongTermLogger
from esa.utils.special_types import JSON_ro
...
def _convert_litellm_message_to_langchain_message(
    ...
    from esa.llm.litellm_singleton import litellm
...
class LitellmLLM(LLM):
    ...
    def _get_thinking_param(
        ...
        is_reasoning: bool,
        tool_choice_formatted: dict | str | None,
    ) -> dict:
        """Get thinking parameter for Claude models.
        
        Bedrock doesn't allow thinking + forced tool_choice together,
        so we skip thinking when a specific tool is forced.
        """
        if not reasoning_effort or not is_reasoning:
            return {}
        
        if "claude" not in self.config.model_name.lower():
            return {}
        
        # Skip thinking if tool_choice forces a specific tool (not 'auto' or 'none')
        # UPDATE: Bedrock throws "Thinking may not be enabled when tool_choice forces tool use"
        # even for 'auto' in some cases (or 'any'). Disabling thinking if ANY tool choice is present to be safe.
        if tool_choice_formatted:
            # Forced tool call or tool use active - don't enable thinking
            return {}
        
        return {"thinking": {"type": "enabled", "budget_tokens": 10000}}

    def _get_reasoning_effort_param(
        self,
        reasoning_effort: str | None,
        is_reasoning: bool,
    ) -> dict:
        """Get reasoning_effort parameter for non-Claude models."""
        if not reasoning_effort or not is_reasoning:
            return {}
        ...
        try:
            return litellm.completion(
                ...
                **(
                ...
                # NOTE: Bedrock doesn't allow thinking + forced tool_choice together
                **self._get_thinking_param(reasoning_effort, is_reasoning, tool_choice_formatted),
                # OpenAI and other providers use reasoning_effort
                # (litellm maps this to thinking_level for Gemini 3 models)
                **self._get_reasoning_effort_param(reasoning_effort, is_reasoning),
                **(
    ...
    def _invoke_implementation(
        ...
        from esa.llm.model_response import from_litellm_model_response
    ...
    def _stream_implementation(
        ...
        from esa.llm.model_response import from_litellm_model_response_stream
