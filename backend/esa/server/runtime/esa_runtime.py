...
from esa.configs.constants import CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT
from esa.configs.constants import ONYX_CLOUD_REDIS_RUNTIME
from esa.configs.constants import ONYX_CLOUD_TENANT_ID
from esa.configs.constants import ONYX_EMAILABLE_LOGO_MAX_DIM
...
from esa.utils.file import ESAStaticFileManager
...
class ESARuntime:
    ...
    def _get_with_static_fallback(
        ...
        esa_file: FileWithMimeType | None = None

        if db_filename:
            file_store = get_default_file_store()
            esa_file = file_store.get_file_with_mime_type(db_filename)

        if not esa_file:
            esa_file = ESAStaticFileManager.get_static(static_filename)
    ...
    def get_logo() -> FileWithMimeType:
        ...
        db_filename: str | None = fetch_ee_implementation_or_noop(
            "esa.server.enterprise_settings.store", "get_logo_filename", None
        )

        return ESARuntime._get_with_static_fallback(db_filename, STATIC_FILENAME)

    @staticmethod
    def get_emailable_logo() -> FileWithMimeType:
        esa_file = ESARuntime.get_logo()

        # check dimensions and resize downwards if necessary or if not PNG
        image = Image.open(io.BytesIO(esa_file.data))
        if (
    ...
    def get_logotype() -> FileWithMimeType:
        ...
        db_filename: str | None = fetch_ee_implementation_or_noop(
            "esa.server.enterprise_settings.store", "get_logotype_filename", None
        )
