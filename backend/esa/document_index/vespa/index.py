...
from esa.configs.app_configs import BLURB_SIZE
from esa.configs.chat_configs import DOC_TIME_DECAY
from esa.configs.chat_configs import NUM_RETURNED_HITS
from esa.configs.chat_configs import TITLE_CONTENT_RATIO
from esa.configs.chat_configs import VESPA_SEARCHER_THREADS
from esa.configs.constants import KV_REINDEX_KEY
from esa.configs.constants import RETURN_SEPARATOR
from esa.context.search.models import IndexFilters
from esa.context.search.models import InferenceChunk
from esa.context.search.models import InferenceChunkUncleaned
from esa.context.search.models import QueryExpansionType
from esa.db.enums import EmbeddingPrecision
from esa.document_index.document_index_utils import get_document_chunk_ids
from esa.document_index.document_index_utils import get_uuid_from_chunk_info
from esa.document_index.interfaces import DocumentIndex
from esa.document_index.interfaces import DocumentInsertionRecord
from esa.document_index.interfaces import EnrichedDocumentIndexingInfo
from esa.document_index.interfaces import IndexBatchParams
from esa.document_index.interfaces import MinimalDocumentIndexingInfo
from esa.document_index.interfaces import UpdateRequest
from esa.document_index.interfaces import VespaChunkRequest
from esa.document_index.interfaces import VespaDocumentFields
from esa.document_index.interfaces import VespaDocumentUserFields
from esa.document_index.vespa.chunk_retrieval import batch_search_api_retrieval
from esa.document_index.vespa.chunk_retrieval import (
    parallel_visit_api_retrieval,
)
from esa.document_index.vespa.chunk_retrieval import query_vespa
from esa.document_index.vespa.deletion import delete_vespa_chunks
from esa.document_index.vespa.indexing_utils import BaseHTTPXClientContext
from esa.document_index.vespa.indexing_utils import batch_index_vespa_chunks
from esa.document_index.vespa.indexing_utils import check_for_final_chunk_existence
from esa.document_index.vespa.indexing_utils import clean_chunk_id_copy
from esa.document_index.vespa.indexing_utils import GlobalHTTPXClientContext
from esa.document_index.vespa.indexing_utils import TemporaryHTTPXClientContext
from esa.document_index.vespa.shared_utils.utils import get_vespa_http_client
from esa.document_index.vespa.shared_utils.utils import (
    replace_invalid_doc_id_characters,
)
from esa.document_index.vespa.shared_utils.vespa_request_builders import (
    build_vespa_filters,
)
from esa.document_index.vespa_constants import ACCESS_CONTROL_LIST
from esa.document_index.vespa_constants import BATCH_SIZE
from esa.document_index.vespa_constants import BOOST
from esa.document_index.vespa_constants import CONTENT_SUMMARY
from esa.document_index.vespa_constants import DOCUMENT_ID
from esa.document_index.vespa_constants import DOCUMENT_ID_ENDPOINT
from esa.document_index.vespa_constants import DOCUMENT_SETS
from esa.document_index.vespa_constants import HIDDEN
from esa.document_index.vespa_constants import NUM_THREADS
from esa.document_index.vespa_constants import USER_PROJECT
from esa.document_index.vespa_constants import VESPA_APPLICATION_ENDPOINT
from esa.document_index.vespa_constants import VESPA_TIMEOUT
from esa.document_index.vespa_constants import YQL_BASE
from esa.indexing.models import DocMetadataAwareIndexChunk
from esa.key_value_store.factory import get_shared_kv_store
from esa.kg.utils.formatting_utils import split_relationship_id
from esa.utils.batching import batch_generator
from esa.utils.logger import setup_logger
from esa.utils.timing import log_function_time
...
class VespaIndex(DocumentIndex):
    ...
    def ensure_indices_exist(
        ...
        vespa_schema_path = os.path.join(
            os.getcwd(), "esa", "document_index", "vespa", "app_config"
        )
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to prepare Vespa ESA Index. Response: {response.text}"
            )
            raise RuntimeError(
                f"Failed to prepare Vespa ESA Index. Response: {response.text}"
            )
    ...
    def register_multitenant_indices(
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"Failed to prepare Vespa ESA Indexes. Response: {response.text}"
            )
