...
from esa.chat.models import DocumentRelevance
...
from esa.context.search.models import SearchDoc as ServerSearchDoc
from esa.db.models import ChatMessage
from esa.db.models import ChatMessage__SearchDoc
from esa.db.models import ChatSession
from esa.db.models import ChatSessionSharedStatus
from esa.db.models import SearchDoc as DBSearchDoc
from esa.db.models import ToolCall
from esa.db.models import User
from esa.db.persona import get_best_persona_id_for_user
...
# Chat sessions do not include esabot flows
def get_chat_sessions_by_user(
    user_id: UUID | None,
    deleted: bool | None,
    db_session: Session,
    include_esabot_flows: bool = False,
    limit: int = 50,
    ...
    if not include_esabot_flows:
        stmt = stmt.where(ChatSession.esabot_flow.is_(False))
...
def delete_all_chat_sessions_for_user(
    ...
    chat_sessions = (
        ...
        .filter(ChatSession.user_id == user_id, ChatSession.esabot_flow.is_(False))
        .all()
    )
    ...
    if hard_delete:
        ...
        db_session.execute(
            delete(ChatSession).where(
                ChatSession.user_id == user_id, ChatSession.esabot_flow.is_(False)
            )
        )
    else:
        db_session.execute(
            update(ChatSession)
            .where(ChatSession.user_id == user_id, ChatSession.esabot_flow.is_(False))
            .values(deleted=True)
        )
...
def add_search_docs_to_tool_call(
    ...
    Args:
        ...
        db_session: The database session
    """
    from esa.db.models import ToolCall__SearchDoc
