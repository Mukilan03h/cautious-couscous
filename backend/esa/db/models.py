...
from esa.configs.chat_configs import NUM_POSTPROCESSED_RESULTS
from esa.configs.constants import (
    DEFAULT_BOOST,
...
from esa.configs.constants import TokenRateLimitScope
from esa.connectors.models import InputType
from esa.db.enums import ChatSessionSharedStatus
...
from esa.db.pydantic_type import PydanticListType, PydanticType
from esa.kg.models import KGEntityTypeAttributes
from esa.utils.logger import setup_logger
from esa.utils.special_types import JSON_ro
from esa.file_store.models import FileDescriptor
from esa.llm.override_models import LLMOverride
from esa.llm.override_models import PromptOverride
from esa.context.search.enums import RecencyBiasSetting
from esa.kg.models import KGStage
from esa.server.features.mcp.models import MCPConnectionData
from esa.utils.encryption import decrypt_bytes_to_string
from esa.utils.encryption import encrypt_string_to_bytes
from esa.utils.headers import HeaderItemDict
...
class ConnectorCredentialPair(Base):
    ...
    # {"customer_id": "123567", "company_domain": "@esa.app"}
...
class KGEntityExtractionStaging(Base):
    ...
    __table_args__ = (
        # Fixed column names in indexes - prefixed with table name to avoid duplicates
        Index("ix_staging_entity_type_acl", entity_type_id_name, acl),
        Index("ix_staging_entity_name_search", name, entity_type_id_name),
    )
...
class KGRelationshipExtractionStaging(Base):
    ...
    __table_args__ = (
        # Composite primary key
        PrimaryKeyConstraint("id_name", "source_document"),
        # Index for querying relationships by type - prefixed to avoid duplicates
        Index("ix_staging_kg_relationship_type", type),
        # Composite index for source/target queries - prefixed to avoid duplicates
        Index("ix_staging_kg_relationship_nodes", source_node, target_node),
        # Ensure unique relationships between nodes of a specific type
        UniqueConstraint(
            "source_node",
            "target_node",
            "type",
            name="uq_staging_kg_relationship_source_target_type",
        ),
    )
...
class ChatSession(Base):
    ...
    # This chat created by ESABot
    esabot_flow: Mapped[bool] = mapped_column(Boolean, default=False)
...
These models are only used in Enterprise Edition only features in ESA.
They are kept here to simplify the codebase and avoid having different assumptions
on the shape of data being passed around between the MIT and EE versions of ESA.

In the MIT version of ESA, assume these tables are always empty.
"""
...
class User__ExternalUserGroupId(Base):
    ...
    directly adding the ESA user to the doc ACL list"""
