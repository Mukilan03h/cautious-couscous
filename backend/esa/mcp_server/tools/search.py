...
from esa.mcp_server.api import mcp_server
from esa.mcp_server.utils import get_api_server_url
from esa.mcp_server.utils import get_http_client
from esa.mcp_server.utils import get_indexed_sources
from esa.mcp_server.utils import require_access_token
...
async def search_indexed_documents(
    ...
    """
    Search the user's knowledge base indexed in ESA.
    ...
    """
    logger.info(
        f"ESA MCP Server: document search: query='{query}', sources={source_types}, limit={limit}"
    )
    ...
    if time_cutoff:
        try:
            ...
            logger.warning(
                f"ESA MCP Server: Invalid time_cutoff format '{time_cutoff}': {e}. "
    ...
    try:
        ...
        logger.error(
            "ESA MCP Server: Error checking indexed sources: %s",
            e,
            exc_info=True,
        )
        return {
    ...
    if not sources:
        logger.info("ESA MCP Server: No indexed sources available for tenant")
        return {
            "documents": [],
            "total_results": 0,
            "query": query,
            "message": (
                ...
                "through ESA before calling esa_search_documents."
            ),
        }
    ...
    if source_types is not None:
        ...
        for src in source_types:
            try:
                ...
                logger.warning(
                    f"ESA MCP Server: Invalid source type '{src}' - will be ignored by server"
                )
    ...
    try:
        ...
        logger.info(
            f"ESA MCP Server: Internal search returned {len(documents)} results"
        )
        return {
            "documents": documents,
        ...
        logger.error(f"ESA MCP Server: Document search error: {e}", exc_info=True)
        return {
...
async def search_web(
    ...
    ```
    """
    logger.info(f"ESA MCP Server: Web search: query='{query}', limit={limit}")
    ...
    try:
        ...
        logger.error(f"ESA MCP Server: Web search error: {e}", exc_info=True)
        return {
...
async def open_urls(
    ...
    ```
    """
    logger.info(f"ESA MCP Server: Open URL: fetching {len(urls)} URLs")
    ...
    try:
        ...
        logger.error(f"ESA MCP Server: URL fetch error: {e}", exc_info=True)
        return {
