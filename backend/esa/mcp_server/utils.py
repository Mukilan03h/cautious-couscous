"""Utility helpers for the ESA MCP server."""
...
def require_access_token() -> AccessToken:
    ...
    if not access_token:
        raise ValueError(
            "MCP Server requires an ESA access token to authenticate your request"
        )
    return access_token
...
async def get_indexed_sources(
    ...
    try:
        ...
        logger.error(
            "ESA MCP Server: Failed to fetch indexed sources",
            exc_info=True,
        )
        raise
        ...
        logger.error(
            "ESA MCP Server: Unexpected error fetching indexed sources",
            exc_info=True,
        )
