...
from esa.configs.app_configs import DB_YIELD_PER_DEFAULT
...
from esa.db.document_set import construct_document_id_select_by_docset
from esa.redis.redis_object_helper import RedisObjectHelper
...
class RedisDocumentSet(RedisObjectHelper):
    ...
    def set_fence(self, payload: int | None) -> None:
        if payload is None:
            self.redis.srem(ESARedisConstants.ACTIVE_FENCES, self.fence_key)
            self.redis.delete(self.fence_key)
            return

        self.redis.set(self.fence_key, payload)
        self.redis.sadd(ESARedisConstants.ACTIVE_FENCES, self.fence_key)
    ...
    def generate_tasks(
        ...
        for doc_id in db_session.scalars(stmt).yield_per(DB_YIELD_PER_DEFAULT):
            ...
            celery_app.send_task(
                ESACeleryTask.VESPA_METADATA_SYNC_TASK,
                kwargs=dict(document_id=doc_id, tenant_id=tenant_id),
                queue=ESACeleryQueues.VESPA_METADATA_SYNC,
