...
from esa.configs.constants import DocumentIndexType
from esa.configs.constants import QueryHistoryType
from esa.file_processing.enums import HtmlBasedConnectorTransformLinksStrategy
from esa.prompts.image_analysis import DEFAULT_IMAGE_ANALYSIS_SYSTEM_PROMPT
from esa.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_SYSTEM_PROMPT
from esa.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_USER_PROMPT
...
# NOTE: if you are having problems accessing the ESA web UI locally (especially
...
# restrict access to ESA to only users with emails from those domains.
# E.g. `VALID_EMAIL_DOMAINS=example.com,example.org` will restrict ESA
...
SAML_CONF_DIR = os.environ.get("SAML_CONF_DIR") or "/app/esa/configs/saml_config"
...
# If set, ESA will listen to the `expires_at` returned by the identity
...
VESPA_DEPLOYMENT_ZIP = (
    os.environ.get("VESPA_DEPLOYMENT_ZIP") or "/app/esa/vespa-app.zip"
)
...
# View the list here:
# https://github.com/esa-dot-app/esa/blob/main/backend/esa/connectors/factory.py
...
# Logs ESA only model interactions like prompts, responses, messages etc.
...
BRAINTRUST_PROJECT = os.environ.get("BRAINTRUST_PROJECT", "ESA")
...
# founders@esa.app OR message Chris Weaver or Yuhong Sun in the ESA
...
S3_FILE_STORE_BUCKET_NAME = (
    os.environ.get("S3_FILE_STORE_BUCKET_NAME") or "esa-file-store-bucket"
)
S3_FILE_STORE_PREFIX = os.environ.get("S3_FILE_STORE_PREFIX") or "esa-files"
