...
import esa.background.celery.apps.app_base as app_base
from esa.background.celery.apps.app_base import task_logger
from esa.background.celery.celery_utils import celery_is_worker_primary
from esa.background.celery.tasks.vespa.document_sync import reset_document_sync
from esa.configs.app_configs import CELERY_WORKER_PRIMARY_POOL_OVERFLOW
from esa.configs.constants import CELERY_PRIMARY_WORKER_LOCK_TIMEOUT
from esa.configs.constants import ESARedisConstants
from esa.configs.constants import ESARedisLocks
from esa.configs.constants import POSTGRES_CELERY_WORKER_PRIMARY_APP_NAME
...
from esa.redis.redis_connector_delete import RedisConnectorDelete
...
from esa.redis.redis_connector_stop import RedisConnectorStop
...
celery_app.config_from_object("esa.background.celery.configs.primary")
...
def on_worker_init(sender: Worker, **kwargs: Any) -> None:
    ...
    r.delete(ESARedisLocks.PRIMARY_WORKER)
    ...
    r.delete(ESARedisLocks.CHECK_VESPA_SYNC_BEAT_LOCK)

    r.delete(ESARedisConstants.ACTIVE_FENCES)
...
celery_app.autodiscover_tasks(
    [
        "esa.background.celery.tasks.connector_deletion",
        "esa.background.celery.tasks.docprocessing",
        "esa.background.celery.tasks.evals",
        "esa.background.celery.tasks.periodic",
        "esa.background.celery.tasks.pruning",
        "esa.background.celery.tasks.shared",
        "esa.background.celery.tasks.vespa",
        "esa.background.celery.tasks.llm_model_update",
        "esa.background.celery.tasks.kg_processing",
        "esa.background.celery.tasks.user_file_processing",
    ]
)
