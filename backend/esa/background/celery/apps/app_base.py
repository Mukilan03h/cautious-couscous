...
from esa.background.celery.apps.task_formatters import CeleryTaskColoredFormatter
from esa.background.celery.apps.task_formatters import CeleryTaskPlainFormatter
from esa.background.celery.celery_utils import celery_is_worker_primary
from esa.background.celery.celery_utils import make_probe_path
from esa.background.celery.tasks.vespa.document_sync import DOCUMENT_SYNC_PREFIX
from esa.background.celery.tasks.vespa.document_sync import DOCUMENT_SYNC_TASKSET_KEY
from esa.configs.constants import ONYX_CLOUD_CELERY_TASK_PREFIX
from esa.configs.constants import ESARedisLocks
from esa.db.engine.sql_engine import get_sqlalchemy_engine
from esa.document_index.vespa.shared_utils.utils import wait_for_vespa_with_timeout
...
from esa.utils.logger import ColoredFormatter
from esa.utils.logger import LoggerContextVars
from esa.utils.logger import PlainFormatter
...
def on_secondary_worker_init(sender: Any, **kwargs: Any) -> None:
    ...
    while True:
        if r.exists(ESARedisLocks.PRIMARY_WORKER):
            break
...
class TenantContextFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if tenant_id:
            # Match the 8 character tenant abbreviation used in ESALoggingAdapter
