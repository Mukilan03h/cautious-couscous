...
from esa.background.celery.celery_utils import extract_ids_from_runnable_connector
from esa.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from esa.background.celery.tasks.docprocessing.utils import IndexingCallbackBase
from esa.configs.app_configs import ALLOW_SIMULTANEOUS_PRUNING
from esa.configs.app_configs import JOB_TIMEOUT
from esa.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from esa.configs.constants import CELERY_PRUNING_LOCK_TIMEOUT
...
from esa.db.connector import mark_ccpair_as_pruned
...
from esa.db.document import get_documents_for_connector_credential_pair
...
from esa.db.tag import delete_orphan_tags__no_commit
from esa.redis.redis_connector import RedisConnector
from esa.redis.redis_connector_prune import RedisConnectorPrune
from esa.redis.redis_connector_prune import RedisConnectorPrunePayload
...
from esa.utils.logger import pruning_ctx
...
@shared_task(
    name=ESACeleryTask.CHECK_FOR_PRUNING,
    ignore_result=True,
...
def check_for_pruning(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        ESARedisLocks.CHECK_PRUNE_BEAT_LOCK,
    ...
    try:
        ...
        if not r.exists(ESARedisSignals.BLOCK_PRUNING):
            ...
            r.set(ESARedisSignals.BLOCK_PRUNING, 1, ex=_get_pruning_block_expiration())

        # we want to run this less frequently than the overall task
        lock_beat.reacquire()
        if not r.exists(ESARedisSignals.BLOCK_VALIDATE_PRUNING_FENCES):
            ...
            r.set(
                ESARedisSignals.BLOCK_VALIDATE_PRUNING_FENCES,
                1,
...
def try_creating_prune_generator_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            ESACeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
            kwargs=dict(
            ...
            queue=ESACeleryQueues.CONNECTOR_PRUNING,
...
@shared_task(
    name=ESACeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_pruning_generator_task(
    ...
    lock: RedisLock = r.lock(
        ESARedisLocks.PRUNING_LOCK_PREFIX + f"_{redis_connector.cc_pair_id}",
...
def validate_pruning_fences(
    ...
    queue_len = celery_get_queue_length(ESACeleryQueues.CONNECTOR_DELETION, r_celery)
...
def validate_pruning_fence(
    ...
    found = celery_find_task(
        ...
        ESACeleryQueues.CONNECTOR_PRUNING,
        r_celery,
    )
    if found:
