...
from esa.background.celery.celery_utils import httpx_init_vespa_pool
from esa.background.celery.memory_monitoring import emit_process_memory
from esa.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from esa.background.celery.tasks.docprocessing.heartbeat import start_heartbeat
from esa.background.celery.tasks.docprocessing.heartbeat import stop_heartbeat
from esa.background.celery.tasks.docprocessing.utils import IndexingCallback
from esa.background.celery.tasks.docprocessing.utils import is_in_repeated_error_state
from esa.background.celery.tasks.docprocessing.utils import should_index
from esa.background.celery.tasks.docprocessing.utils import (
    try_creating_docfetching_task,
)
from esa.background.celery.tasks.models import DocProcessingContext
from esa.background.indexing.checkpointing_utils import cleanup_checkpoint
from esa.background.indexing.checkpointing_utils import (
    get_index_attempts_with_old_checkpoints,
)
from esa.background.indexing.index_attempt_utils import cleanup_index_attempts
from esa.background.indexing.index_attempt_utils import get_old_index_attempts
from esa.configs.app_configs import MANAGED_VESPA
from esa.configs.app_configs import VESPA_CLOUD_CERT_PATH
from esa.configs.app_configs import VESPA_CLOUD_KEY_PATH
from esa.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from esa.configs.constants import CELERY_INDEXING_LOCK_TIMEOUT
...
from esa.connectors.models import ConnectorFailure
from esa.connectors.models import Document
from esa.connectors.models import IndexAttemptMetadata
...
from esa.db.connector_credential_pair import set_cc_pair_repeated_error_state
from esa.db.engine.sql_engine import get_session_with_current_tenant
from esa.db.engine.time_utils import get_db_current_time
from esa.db.enums import ConnectorCredentialPairStatus
from esa.db.enums import IndexingMode
from esa.db.enums import IndexingStatus
from esa.db.enums import SwitchoverType
from esa.db.index_attempt import create_index_attempt_error
from esa.db.index_attempt import get_index_attempt
from esa.db.index_attempt import get_index_attempt_errors_for_cc_pair
from esa.db.index_attempt import IndexAttemptError
from esa.db.index_attempt import mark_attempt_canceled
from esa.db.index_attempt import mark_attempt_failed
from esa.db.index_attempt import mark_attempt_partially_succeeded
from esa.db.index_attempt import mark_attempt_succeeded
from esa.db.indexing_coordination import CoordinationStatus
from esa.db.indexing_coordination import INDEXING_PROGRESS_TIMEOUT_HOURS
from esa.db.indexing_coordination import IndexingCoordination
from esa.db.models import IndexAttempt
from esa.db.models import SearchSettings
from esa.db.search_settings import get_current_search_settings
from esa.db.search_settings import get_secondary_search_settings
from esa.db.swap_index import check_and_perform_index_swap
from esa.document_index.factory import get_default_document_index
from esa.file_store.document_batch_storage import DocumentBatchStorage
from esa.file_store.document_batch_storage import get_document_batch_storage
from esa.httpx.httpx_pool import HttpxPool
from esa.indexing.adapters.document_indexing_adapter import (
    DocumentIndexingBatchAdapter,
)
from esa.indexing.embedder import DefaultIndexingEmbedder
from esa.indexing.indexing_pipeline import run_indexing_pipeline
from esa.natural_language_processing.search_nlp_models import EmbeddingModel
from esa.natural_language_processing.search_nlp_models import (
    InformationContentClassificationModel,
)
from esa.natural_language_processing.search_nlp_models import warm_up_bi_encoder
...
from esa.redis.redis_pool import SCAN_ITER_COUNT_DEFAULT
from esa.redis.redis_utils import is_fence
from esa.server.runtime.esa_runtime import ESARuntime
from esa.utils.logger import setup_logger
from esa.utils.middleware import make_randomized_esa_request_id
        ...
        if attempt and timed_out:
            if attempt.status == IndexingStatus.IN_PROGRESS:
                ...
                task_exists = celery_find_task(
                    ...
                    ESACeleryQueues.CONNECTOR_DOC_FETCHING,
                    redis_celery,
                )
                unacked_task_ids = celery_get_unacked_task_ids(
                    ESACeleryQueues.CONNECTOR_DOC_FETCHING, redis_celery
                )
...
@shared_task(
    name=ESACeleryTask.CHECK_FOR_INDEXING,
    soft_time_limit=300,
    bind=True,
)
def check_for_indexing(self: Task, *, tenant_id: str) -> int | None:
    ...
    the docfetching task (ESACeleryTask.CONNECTOR_DOC_FETCHING_TASK).
    ...
    lock_beat: RedisLock = redis_client.lock(
        ESARedisLocks.CHECK_INDEXING_BEAT_LOCK,
    ...
    try:
        ...
        if not redis_client.exists(ESARedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE):
            ...
            for key_bytes in redis_client_replica.scan_iter(
                ...
                if is_fence(key_bytes) and not redis_client.sismember(
                    ESARedisConstants.ACTIVE_FENCES, key_bytes
                ):
                    logger.warning(f"Adding {key_bytes} to the lookup table.")
                    redis_client.sadd(ESARedisConstants.ACTIVE_FENCES, key_bytes)

            redis_client.set(
                ESARedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE,
                1,
                ex=ESARuntime.get_build_fence_lookup_table_interval(),
            )
        ...
        lock_beat.reacquire()
        # we want to run this less frequently than the overall task
        if not redis_client.exists(ESARedisSignals.BLOCK_VALIDATE_INDEXING_FENCES):
            ...
            redis_client.set(
                ESARedisSignals.BLOCK_VALIDATE_INDEXING_FENCES,
                1,
...
# primary
@shared_task(
    name=ESACeleryTask.CHECK_FOR_CHECKPOINT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_checkpoint_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        ESARedisLocks.CHECK_CHECKPOINT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for attempt in old_attempts:
                ...
                self.app.send_task(
                    ESACeleryTask.CLEANUP_CHECKPOINT,
                    kwargs={
                        "index_attempt_id": attempt.id,
                        "tenant_id": tenant_id,
                    },
                    queue=ESACeleryQueues.CHECKPOINT_CLEANUP,
...
# light worker
@shared_task(
    name=ESACeleryTask.CLEANUP_CHECKPOINT,
    bind=True,
)
...
# primary
@shared_task(
    name=ESACeleryTask.CHECK_FOR_INDEX_ATTEMPT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_index_attempt_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        ESARedisLocks.CHECK_INDEX_ATTEMPT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for i in range(0, len(old_attempts), batch_size):
                ...
                self.app.send_task(
                    ESACeleryTask.CLEANUP_INDEX_ATTEMPT,
                    kwargs={
                        "index_attempt_ids": [attempt.id for attempt in batch],
                        "tenant_id": tenant_id,
                    },
                    queue=ESACeleryQueues.INDEX_ATTEMPT_CLEANUP,
                    priority=ESACeleryPriority.MEDIUM,
                )
    except Exception:
...
# light worker
@shared_task(
    name=ESACeleryTask.CLEANUP_INDEX_ATTEMPT,
    bind=True,
)
...
@shared_task(
    name=ESACeleryTask.DOCPROCESSING_TASK,
    bind=True,
)
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            index_attempt_metadata = IndexAttemptMetadata(
                ...
                request_id=make_randomized_esa_request_id("DIP"),
