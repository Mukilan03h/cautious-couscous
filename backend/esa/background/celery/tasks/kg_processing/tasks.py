...
from esa.kg.clustering.clustering import kg_clustering
from esa.kg.extractions.extraction_processing import kg_extraction
from esa.kg.resets.reset_source import reset_source_kg_index
...
@shared_task(
    name=ESACeleryTask.CHECK_KG_PROCESSING,
    soft_time_limit=300,
    bind=True,
)
def check_for_kg_processing(self: Task, *, tenant_id: str) -> None:
    ...
    try:
        ...
        self.app.send_task(
            ESACeleryTask.KG_PROCESSING,
            kwargs={
                "tenant_id": tenant_id,
            },
            queue=ESACeleryQueues.KG_PROCESSING,
...
@shared_task(
    name=ESACeleryTask.CHECK_KG_PROCESSING_CLUSTERING_ONLY,
    soft_time_limit=300,
    bind=True,
)
def check_for_kg_processing_clustering_only(self: Task, *, tenant_id: str) -> None:
    ...
    try:
        ...
        self.app.send_task(
            ESACeleryTask.KG_CLUSTERING_ONLY,
            kwargs={
                "tenant_id": tenant_id,
            },
            queue=ESACeleryQueues.KG_PROCESSING,
...
@shared_task(
    name=ESACeleryTask.KG_PROCESSING,
    bind=True,
)
def kg_processing(self: Task, *, tenant_id: str) -> None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        ESARedisLocks.KG_PROCESSING_LOCK,
...
@shared_task(
    name=ESACeleryTask.KG_CLUSTERING_ONLY,
    bind=True,
)
...
@shared_task(
    name=ESACeleryTask.KG_RESET_SOURCE_INDEX,
    bind=True,
)
