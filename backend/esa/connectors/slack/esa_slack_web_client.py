...
from esa.connectors.slack.utils import ESA_SLACK_LOCK_BLOCKING_TIMEOUT
from esa.connectors.slack.utils import ESA_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT
from esa.connectors.slack.utils import ESA_SLACK_LOCK_TTL
from esa.utils.logger import setup_logger

logger = setup_logger()


class ESASlackWebClient(WebClient):
    """Use in combination with the ESA Retry Handler.
    ...
    def _perform_urllib_http_request(
        ...
        lock: RedisLock = self._redis.lock(
            self._delay_lock,
            timeout=ESA_SLACK_LOCK_TTL,
        )

        # try to acquire the lock
        start = time.monotonic()
        while True:
            acquired = lock.acquire(blocking_timeout=ESA_SLACK_LOCK_BLOCKING_TIMEOUT)
            if acquired:
                break
            ...
            if time.monotonic() - start > ESA_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT:
                raise RuntimeError(
                    f"ESASlackWebClient._perform_urllib_http_request - "
                    f"timed out waiting for lock: {ESA_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT=}"
                )
        ...
        try:
            ...
            if lock.owned():
                ...
                logger.warning(
                    "ESASlackWebClient._perform_urllib_http_request lock not owned on release"
                )

        time.monotonic() - start
        # logger.info(
        #     f"ESASlackWebClient._perform_urllib_http_request: Releasing lock: {elapsed=}"
        # )
    ...
    def _perform_urllib_http_request_internal(
        ...
        if delay_ms > 0:
            logger.warning(
                f"ESASlackWebClient._perform_urllib_http_request_internal delay: "
                f"{delay_ms=} "
                f"{self.num_requests=}"
            )
