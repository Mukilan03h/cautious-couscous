...
from esa.configs.app_configs import NOTION_CONNECTOR_DISABLE_RECURSIVE_PAGE_LOOKUP
from esa.configs.constants import DocumentSource
from esa.connectors.cross_connector_utils.rate_limit_wrapper import (
    rl_requests,
)
        ...
        try:
            ...
            if res.status_code == 404:
                ...
                logger.error(
                    ...
                    f"with the ESA integration. Exact exception:\n\n{e}"
                )
            else:
                logger.exception(
            ...
            for result in data["results"]:
                ...
                if result_type == "ai_block":
                    logger.warning(
                        ...
                        f"(discussion: https://github.com/esa-dot-app/esa/issues/1053)"
                    )
                    continue
                ...
                if result_type == "unsupported":
                    logger.warning(
                        ...
                        f"(discussion: https://github.com/esa-dot-app/esa/issues/1230)"
                    )
                    continue
                ...
                if result_type == "external_object_instance_page":
                    logger.warning(
                        ...
                        f"(discussion: https://github.com/esa-dot-app/esa/issues/1761)"
                    )
                    continue
