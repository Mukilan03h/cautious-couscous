...
from esa.connectors.interfaces import GenerateDocumentsOutput
from esa.connectors.interfaces import GenerateSlimDocumentOutput
from esa.connectors.interfaces import LoadConnector
from esa.connectors.interfaces import PollConnector
...
from esa.connectors.salesforce.doc_conversion import convert_sf_object_to_doc
from esa.connectors.salesforce.doc_conversion import convert_sf_query_result_to_doc
from esa.connectors.salesforce.doc_conversion import ID_PREFIX
from esa.connectors.salesforce.esa_salesforce import ESASalesforce
from esa.connectors.salesforce.salesforce_calls import fetch_all_csvs_in_parallel
from esa.connectors.salesforce.sqlite_functions import ESASalesforceSQLite
from esa.connectors.salesforce.utils import ACCOUNT_OBJECT_TYPE
from esa.connectors.salesforce.utils import BASE_DATA_PATH
from esa.connectors.salesforce.utils import get_sqlite_db_path
from esa.connectors.salesforce.utils import ID_FIELD
from esa.connectors.salesforce.utils import MODIFIED_FIELD
from esa.connectors.salesforce.utils import NAME_FIELD
from esa.connectors.salesforce.utils import USER_OBJECT_TYPE
...
class SalesforceConnector(LoadConnector, PollConnector, SlimConnectorWithPermSync):
    ...
    def __init__(
        ...
        self._sf_client: ESASalesforce | None = None
    ...
    @property
    def sf_client(self) -> ESASalesforce:
    ...
    def _load_csvs_to_db(
        csv_directory: str, remove_ids: bool, sf_db: ESASalesforceSQLite
    ) -> dict[str, str]:
        """
        ...
        # from import esa.connectors.salesforce.sf_db.sqlite_functions find_ids_by_type
    ...
    #     parent_types: list[str], sf_client: ESASalesforce
    # ) -> set[str]:
    ...
    def _full_sync(
        ...
        sf_db = ESASalesforceSQLite(os.path.join(temp_dir, "salesforce_db.sqlite"))
        sf_db.connect()
    ...
    def _make_context(
        ...
        for parent_type in parent_types:
            # parent_esa_sf_type = ESASalesforceType(parent_type, sf_client)
            ...
            for child_type, child_relationship in child_types_working.items():
                ...
                # esa_sf_type = ESASalesforceType(child_type, sf_client)
        ...
        for sf_type in all_types:
            # esa_sf_type = ESASalesforceType(sf_type, sf_client)
