...
def _process_file(
    ...
    if not is_accepted_file_ext(extension, ESAExtensionType.All):
        logger.warning(
    ...
    esa_metadata, custom_tags = process_esa_metadata(metadata)
    file_display_name = esa_metadata.file_display_name or os.path.basename(file_name)
    time_updated = esa_metadata.doc_updated_at or datetime.now(timezone.utc)
    primary_owners = esa_metadata.primary_owners
    secondary_owners = esa_metadata.secondary_owners
    link = esa_metadata.link

    # These metadata items are not settable by the user
    source_type = esa_metadata.source_type or DocumentSource.FILE
    ...
    # Each file may have file-specific ONYX_METADATA https://docs.esa.app/admins/connectors/official/file
    ...
    if extraction_result.metadata:
        ...
        esa_metadata, more_custom_tags = process_esa_metadata(
        ...
        source_type = esa_metadata.source_type or source_type
        primary_owners = esa_metadata.primary_owners or primary_owners
        secondary_owners = esa_metadata.secondary_owners or secondary_owners
        time_updated = esa_metadata.doc_updated_at or time_updated
        file_display_name = esa_metadata.file_display_name or file_display_name
        title = esa_metadata.title or esa_metadata.file_display_name or title
