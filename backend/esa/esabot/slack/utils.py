...
from esa.configs.app_configs import DISABLE_TELEMETRY
from esa.configs.constants import ID_SEPARATOR
from esa.configs.constants import MessageType
from esa.configs.esabot_configs import ONYX_BOT_FEEDBACK_VISIBILITY
from esa.configs.esabot_configs import ONYX_BOT_MAX_QPM
from esa.configs.esabot_configs import ONYX_BOT_MAX_WAIT_TIME
from esa.configs.esabot_configs import ONYX_BOT_NUM_RETRIES
from esa.configs.esabot_configs import (
...
from esa.llm.utils import dict_based_prompt_to_langchain_prompt
from esa.llm.utils import message_to_string
from esa.esabot.slack.constants import FeedbackVisibility
from esa.esabot.slack.models import ChannelType
from esa.esabot.slack.models import ThreadMessage
from esa.prompts.miscellaneous_prompts import SLACK_LANGUAGE_REPHRASE_PROMPT
from esa.utils.logger import setup_logger
from esa.utils.telemetry import optional_telemetry
from esa.utils.telemetry import RecordType
from esa.utils.text_processing import replace_whitespaces_w_space
...
def check_message_limit() -> bool:
    ...
    if (_ONYX_BOT_MESSAGE_COUNT + 1) > ONYX_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD:
        logger.error(
            f"ESABot has reached the message limit {ONYX_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD}"
            f" for the time period {ONYX_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS} seconds."
            " These limits are configurable in backend/esa/configs/esabot_configs.py"
        )
        return False
...
def remove_esa_bot_tag(tenant_id: str, message_str: str, client: WebClient) -> str:
    bot_token_user_id, _ = get_esa_bot_auth_ids(tenant_id, web_client=client)
...
def _build_error_block(error_message: str) -> Block:
    ...
    """
    display_text = (
        "There was an error displaying all of the ESA answers."
        f" Please let an admin or an esa developer know. Error: {error_message}"
    )
    ...
    for reply in replies:
        if "user" in reply and "bot_id" not in reply:
            ...
            if is_esa_bot_response:
                ...
                if not blocks:
                    logger.warning(f"ESABot response has no blocks: {reply}")
                    continue
                ...
                if message is not None and message.startswith("_Filters"):
                    if len(blocks) < 2:
                        ...
                        continue
                    # This is the ESABot answer format, if there is a change to how we respond,
                    # this will need to be updated to get the correct "answer" portion
                    message = reply["blocks"][1].get("text", {}).get("text")
            else:
                # Other bots are not counted as the LLM response which only comes from ESA
                message_type = MessageType.USER
        ...
        message = remove_esa_bot_tag(tenant_id, message, client=client)
...
def slack_usage_report(action: str, sender_id: str | None, client: WebClient) -> None:
    ...
    if sender_email is not None:
        with get_session_with_current_tenant() as db_session:
            esa_user = get_user_by_email(email=sender_email, db_session=db_session)

    optional_telemetry(
        record_type=RecordType.USAGE,
        data={"action": action},
        user_id=str(esa_user.id) if esa_user else "Non-ESA-Or-No-Auth-User",
    )
