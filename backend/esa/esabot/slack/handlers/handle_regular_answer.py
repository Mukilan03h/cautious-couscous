...
from esa.configs.esabot_configs import ONYX_BOT_DISABLE_DOCS_ONLY_ANSWER
from esa.configs.esabot_configs import ONYX_BOT_DISPLAY_ERROR_MSGS
from esa.configs.esabot_configs import ONYX_BOT_NUM_RETRIES
from esa.configs.esabot_configs import ONYX_BOT_REACT_EMOJI
from esa.context.search.enums import OptionalSearchSetting
from esa.context.search.models import BaseFilters
from esa.context.search.models import RetrievalDetails
from esa.db.engine.sql_engine import get_session_with_current_tenant
from esa.db.models import SlackChannelConfig
...
from esa.esabot.slack.handlers.utils import send_team_member_message
...
from esa.esabot.slack.utils import SlackRateLimiter
...
from esa.utils.logger import ESALoggingAdapter
...
def handle_regular_answer(
    ...
    # the message is a dm with the ESA bot.
    send_as_ephemeral = (
    ...
    # or the message is a dm to the ESA bot, we should use the proper esa user from the email.
    # This will make documents privately accessible to the user available to ESA Bot answers.
    # Otherwise - if not ephemeral or DM to ESA Bot - we must use None as the user to restrict
    # to public docs.
    ...
    if not message_ts_to_respond_to and not is_slash_command:
        # if the message is not "/esa" command, then it should have a message ts to respond to
        raise RuntimeError(
    ...
    try:
        ...
        answer = _get_slack_answer(
            ...
            esa_user=user if can_search_over_private_docs else None,
    ...
    try:
        ...
        # if there is no message_ts_to_respond_to, and we have made it this far, then this is a /esa message
