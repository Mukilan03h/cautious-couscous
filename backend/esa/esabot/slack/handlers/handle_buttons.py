...
from esa.chat.process_message import remove_answer_citations
from esa.configs.constants import MessageType
from esa.configs.constants import SearchFeedbackType
from esa.configs.esabot_configs import ONYX_BOT_FOLLOWUP_EMOJI
from esa.connectors.slack.utils import expert_info_from_slack_id
from esa.context.search.models import SavedSearchDoc
...
from esa.esabot.slack.blocks import build_follow_up_resolved_blocks
from esa.esabot.slack.blocks import build_slack_response_blocks
from esa.esabot.slack.blocks import get_document_feedback_blocks
...
from esa.esabot.slack.constants import FeedbackVisibility
...
from esa.esabot.slack.handlers.handle_regular_answer import (
    handle_regular_answer,
)
from esa.esabot.slack.models import SlackMessageInfo
from esa.esabot.slack.utils import build_feedback_id
from esa.esabot.slack.utils import decompose_action_id
from esa.esabot.slack.utils import fetch_group_ids_from_names
from esa.esabot.slack.utils import fetch_slack_user_ids_from_emails
from esa.esabot.slack.utils import get_channel_name_from_id
from esa.esabot.slack.utils import get_feedback_visibility
...
from esa.esabot.slack.utils import update_emote_react
from esa.server.query_and_chat.models import ChatMessageDetail
...
def handle_publish_ephemeral_message_button(
    ...
    # Obtain esa_user and chat_message information
    if not chat_message_id:
        raise ValueError("Missing chat_message_id in the payload")

    with get_session_with_current_tenant() as db_session:
        esa_user = get_user_by_email(user_email, db_session)
        if not esa_user:
            raise ValueError("Cannot determine esa_user_id from email in payload")
        try:
            chat_message = get_chat_message(chat_message_id, esa_user.id, db_session)
        except ValueError:
        ...
        esa_bot_answer = ChatBasicResponse(
    ...
    if action_id == SHOW_EVERYONE_ACTION_ID:
        ...
        try:
            ...
            respond_in_thread_or_channel(
                ...
                text="Hello! ESA has some results for you!",
                blocks=all_blocks,
