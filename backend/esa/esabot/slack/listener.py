...
from esa.chat.models import ThreadMessage
from esa.configs.app_configs import DEV_MODE
from esa.configs.app_configs import POD_NAME
from esa.configs.app_configs import POD_NAMESPACE
from esa.configs.constants import MessageType
from esa.configs.constants import ESARedisLocks
from esa.configs.esabot_configs import NOTIFY_SLACKBOT_NO_ANSWER
from esa.configs.esabot_configs import ONYX_BOT_REPHRASE_MESSAGE
from esa.connectors.slack.utils import expert_info_from_slack_id
from esa.context.search.retrieval.search_runner import (
    download_nltk_data,
)
...
from esa.db.models import SlackBot
from esa.db.search_settings import get_current_search_settings
from esa.db.slack_bot import fetch_slack_bot
from esa.db.slack_bot import fetch_slack_bots
...
from esa.esabot.slack.config import get_slack_channel_config_for_bot_and_channel
from esa.esabot.slack.config import MAX_TENANTS_PER_POD
from esa.esabot.slack.config import TENANT_ACQUISITION_INTERVAL
from esa.esabot.slack.config import TENANT_HEARTBEAT_EXPIRATION
from esa.esabot.slack.config import TENANT_HEARTBEAT_INTERVAL
from esa.esabot.slack.config import TENANT_LOCK_EXPIRATION
from esa.esabot.slack.constants import DISLIKE_BLOCK_ACTION_ID
from esa.esabot.slack.constants import FEEDBACK_DOC_BUTTON_BLOCK_ACTION_ID
from esa.esabot.slack.constants import FOLLOWUP_BUTTON_ACTION_ID
from esa.esabot.slack.constants import FOLLOWUP_BUTTON_RESOLVED_ACTION_ID
from esa.esabot.slack.constants import GENERATE_ANSWER_BUTTON_ACTION_ID
from esa.esabot.slack.constants import IMMEDIATE_RESOLVED_BUTTON_ACTION_ID
from esa.esabot.slack.constants import KEEP_TO_YOURSELF_ACTION_ID
from esa.esabot.slack.constants import LIKE_BLOCK_ACTION_ID
from esa.esabot.slack.constants import SHOW_EVERYONE_ACTION_ID
from esa.esabot.slack.constants import VIEW_DOC_FEEDBACK_ID
from esa.esabot.slack.handlers.handle_buttons import handle_doc_feedback_button
from esa.esabot.slack.handlers.handle_buttons import handle_followup_button
from esa.esabot.slack.handlers.handle_buttons import (
...
from esa.esabot.slack.handlers.handle_buttons import handle_slack_feedback
from esa.esabot.slack.handlers.handle_message import handle_message
from esa.esabot.slack.handlers.handle_message import (
    remove_scheduled_feedback_reminder,
)
from esa.esabot.slack.handlers.handle_message import schedule_feedback_reminder
from esa.esabot.slack.models import SlackContext
from esa.esabot.slack.models import SlackMessageInfo
from esa.esabot.slack.utils import check_message_limit
from esa.esabot.slack.utils import decompose_action_id
from esa.esabot.slack.utils import get_channel_name_from_id
from esa.esabot.slack.utils import get_channel_type_from_id
from esa.esabot.slack.utils import get_esa_bot_auth_ids
from esa.esabot.slack.utils import read_slack_thread
from esa.esabot.slack.utils import remove_esa_bot_tag
from esa.esabot.slack.utils import rephrase_slack_message
from esa.esabot.slack.utils import respond_in_thread_or_channel
from esa.esabot.slack.utils import TenantSocketModeClient
from esa.redis.redis_pool import get_redis_client
from esa.server.manage.models import SlackBotTokens
...
class SlackbotHandler:
    ...
    def acquire_tenants(self) -> None:
        ...
        gated_tenants = fetch_ee_implementation_or_noop(
            "esa.server.tenants.product_gating",
            "get_gated_tenants",
            set(),
        )()
        all_active_tenants = [
    ...
    def send_heartbeats(pod_id: str, tenant_ids: set[str]) -> None:
        ...
        for tenant_id in tenant_ids:
            redis_client = get_redis_client(tenant_id=tenant_id)
            heartbeat_key = f"{ESARedisLocks.SLACK_BOT_HEARTBEAT_PREFIX}:{pod_id}"
            redis_client.set(
...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    bot_token_user_id, bot_token_bot_id = get_esa_bot_auth_ids(
    ...
    if req.type == "events_api":
        ...
        if (
            ...
            or remove_esa_bot_tag(tenant_id, msg, client=client.web_client)
        ...
        if event_type == "message":
            ...
            # ESABot should never respond to itself
            if is_esa_bot_msg:
                logger.info("Ignoring message from ESABot (self-message)")
                return False

            # DMs with the bot don't pick up the @ESABot so we have to keep the
            # caught events_api
        ...
        if is_bot_message:
            ...
            # If ESABot is not specifically tagged and the channel is not set to respond to bots, ignore the message
        ...
        # Can respond in thread if it's an "im" directly to ESA or @ESABot is tagged
        if (
            thread_ts
    ...
    if req.type == "slash_commands":
        ...
        if not channel:
            channel_specific_logger.error(
                "Received ESABot command without channel - skipping"
            )
            return False
        ...
        if not sender:
            channel_specific_logger.error(
                "Cannot respond to ESABot command without sender to respond to."
            )
            return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        bot_token_user_id, _ = get_esa_bot_auth_ids(tenant_id, client.web_client)
        message_ts = event.get("ts")
        thread_ts = event.get("thread_ts")
        ...
        msg = remove_esa_bot_tag(tenant_id, msg, client=client.web_client)
        ...
        if tagged:
            logger.debug("User tagged ESABot")
...
def _get_socket_client(
    ...
    # https://docs.esa.app/admins/getting_started/slack_bot_setup
