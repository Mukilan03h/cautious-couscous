...
from esa.file_processing.file_validation import TEXT_MIME_TYPE
...
from esa.file_processing.unstructured import unstructured_to_text
from esa.utils.file_types import PRESENTATION_MIME_TYPE
from esa.utils.file_types import WORD_PROCESSING_MIME_TYPE
...
class ESAExtensionType(IntFlag):
    Plain = auto()
    Document = auto()
    Multimedia = auto()
...
def is_accepted_file_ext(ext: str, ext_type: ESAExtensionType) -> bool:
    if ext_type & ESAExtensionType.Plain:
        if ext in ACCEPTED_PLAIN_TEXT_FILE_EXTENSIONS:
            return True

    if ext_type & ESAExtensionType.Document:
        if ext in ACCEPTED_DOCUMENT_FILE_EXTENSIONS:
            return True

    if ext_type & ESAExtensionType.Multimedia:
...
def _extract_esa_metadata(line: str) -> dict | None:
    """
...
def read_text_file(
    ...
    encoding: str = "utf-8",
    errors: str = "replace",
    ignore_esa_metadata: bool = True,
) -> tuple[str, dict]:
    """
    For plain text files. Optionally extracts ESA metadata from the first line.
    """
    metadata = {}
    file_content_raw = ""
    for ind, line in enumerate(file):
        ...
        if ind == 0 and not ignore_esa_metadata:
            potential_meta = _extract_esa_metadata(line)
...
def docx_to_text_and_images(
    ...
    try:
        ...
        text_content_raw, _ = read_text_file(
            file, encoding=encoding, ignore_esa_metadata=False
        )
