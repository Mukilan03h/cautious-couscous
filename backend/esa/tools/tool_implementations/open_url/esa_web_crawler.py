...
from esa.file_processing.html_utils import ParsedHTML
...
DEFAULT_USER_AGENT = "ESAWebCrawler/1.0 (+https://www.esa.app)"


class ESAWebCrawler(WebContentProvider):
    """
    ...
    def _fetch_url(self, url: str) -> WebContent:
        try:
            ...
            logger.warning(
                "ESA crawler failed to fetch %s (%s)",
                url,
        ...
        if response.status_code >= 400:
            logger.warning("ESA crawler received %s for %s", response.status_code, url)
            return WebContent(
                title="",
                link=url,
                full_content="",
        ...
        try:
            ...
            logger.warning(
                "ESA crawler failed to parse %s (%s)", url, exc.__class__.__name__
            )
            text_content = ""
            title = ""
