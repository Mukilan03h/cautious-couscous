...
This directory contains the evaluation framework for testing and measuring the performance of ESA's chat and retrieval systems.

## Overview

The evaluation system uses [Braintrust](https://www.braintrust.dev/) to run automated evaluations against test datasets. It measures the quality of responses generated by ESA's chat system and can be used to track performance improvements over time.
...
```bash
esa/backend$ python -m dotenv -f .vscode/.env run -- python esa/evals/eval_cli.py --remote --api-key <SUPER_CLOUD_USER_API_KEY> --search-permissions-email <email account to reference> --remote --remote-dataset-name Simple
```

You can also run the CLI directly from the command line:

```bash
esa$ python -m dotenv -f .vscode/.env run -- python backend/esa/evals/eval_cli.py --local-dataset-path backend/esa/evals/data/eval.json --search-permissions-email richard@esa.app
```
