...
class ESARequestIDFilter(logging.Filter):
...
class ESALoggingAdapter(logging.LoggerAdapter):
    def process(
...
def setup_logger(
    ...
    if logger.handlers:
        return ESALoggingAdapter(logger, extra=extra)
    ...
    if LOG_FILE_NAME and (is_containerized or DEV_LOGGING_ENABLED):
        ...
        for level in log_levels:
            file_name = (
                f"/var/log/esa/{LOG_FILE_NAME}_{level}.log"
                if is_containerized
...
def setup_uvicorn_logger(
    ...
    uvicorn_logger.addFilter(ESARequestIDFilter())
