"""
Document Permission Syncing Celery tasks for MyPlatform.
Ported from ee/esa/background/celery/tasks/doc_permission_syncing/tasks.py - FULL VERSION (1035 lines)

This module provides the complete infrastructure for syncing document permissions
from various connector sources (Google Drive, SharePoint, Confluence, etc.)
"""
import time
...
from esa.background.celery.celery_redis import celery_get_queue_length
from esa.background.celery.celery_redis import celery_get_queued_task_ids
...
from esa.configs.constants import CELERY_PERMISSIONS_SYNC_LOCK_TIMEOUT
from esa.configs.constants import CELERY_TASK_WAIT_FOR_FENCE_TIMEOUT
from esa.configs.constants import DANSWER_REDIS_FUNCTION_LOCK_PREFIX
...
from esa.db.document import get_document_ids_for_connector_credential_pair
from esa.db.document import get_documents_for_connector_credential_pair_limited_columns
...
from esa.db.permission_sync_attempt import complete_doc_permission_sync_attempt
from esa.db.permission_sync_attempt import create_doc_permission_sync_attempt
from esa.db.permission_sync_attempt import mark_doc_permission_sync_attempt_failed
...
from esa.db.users import batch_add_ext_perm_user_if_not_exists
from esa.db.utils import DocumentRow
from esa.db.utils import is_retryable_sqlalchemy_error
...
from esa.redis.redis_connector_doc_perm_sync import RedisConnectorPermissionSync
from esa.redis.redis_connector_doc_perm_sync import RedisConnectorPermissionSyncPayload
...
from esa.utils.logger import doc_permission_sync_ctx
from esa.utils.logger import format_error_for_logging
from esa.utils.logger import LoggerContextVars
...
def _is_external_doc_permissions_sync_due(cc_pair: ConnectorCredentialPair) -> bool:
    ...
    source_sync_period *= int(ESARuntime.get_doc_permission_sync_multiplier())
...
@shared_task(
    name=ESACeleryTask.CHECK_FOR_DOC_PERMISSIONS_SYNC,
    ignore_result=True,
...
def check_for_doc_permissions_sync(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        ESARedisLocks.CHECK_CONNECTOR_DOC_PERMISSIONS_SYNC_BEAT_LOCK,
    ...
    try:
        ...
        lock_beat.reacquire()
        if not r.exists(ESARedisSignals.BLOCK_VALIDATE_PERMISSION_SYNC_FENCES):
            ...
            r.set(
                ESARedisSignals.BLOCK_VALIDATE_PERMISSION_SYNC_FENCES,
                1,
                ex=_get_fence_validation_block_expiration(),
            )

        # use a lookup table to find active fences
        lock_beat.reacquire()
        ...
        for key in keys:
            ...
            if not r.exists(key_bytes):
                r.srem(ESARedisConstants.ACTIVE_FENCES, key_bytes)
                continue
...
def try_creating_permissions_sync_task(
    ...
    try:
        ...
        # create before setting fence to avoid race condition
        try:
            with get_session_with_current_tenant() as db_session:
                insert_sync_record(
        ...
        result = app.send_task(
            ESACeleryTask.CONNECTOR_PERMISSION_SYNC_GENERATOR_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair_id,
                tenant_id=tenant_id,
            ),
            queue=ESACeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
...
@shared_task(
    name=ESACeleryTask.CONNECTOR_PERMISSION_SYNC_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_permission_sync_generator_task(
    ...
    while True:
        if time.monotonic() - start > CELERY_TASK_WAIT_FOR_FENCE_TIMEOUT:
            error_msg = (
                f"connector_permission_sync_generator_task - timed out waiting for fence: "
    ...
    lock: RedisLock = r.lock(
        ESARedisLocks.CONNECTOR_DOC_PERMISSIONS_SYNC_LOCK_PREFIX
...
def validate_permission_sync_fences(
    ...
    queue_len = celery_get_queue_length(
        ESACeleryQueues.DOC_PERMISSIONS_UPSERT, r_celery
    )
    ...
    reserved_generator_tasks = celery_get_unacked_task_ids(
        ESACeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC, r_celery
    )
...
def validate_permission_sync_fence(
    ...
    # Check if generator task exists
    found = celery_find_task(
        payload.celery_task_id,
        ESACeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        r_celery,
    )
    if found:
        redis_connector.permissions.set_active()
...
class PermissionSyncCallback(IndexingHeartbeatInterface):
    """Callback for tracking permission sync progress."""
