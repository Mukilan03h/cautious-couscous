...
Ported from ee/esa/db/user_group.py - FULL VERSION (799 lines)

Handles user group CRUD operations, permissions, and sync functionality.
"""
...
def construct_document_id_select_by_usergroup(
    ...
    .yield_per() to minimize overhead."""
    stmt = (
        select(Document.id)
        .join(
...
def _validate_curator_relationship_update_requester(
    ...
    # check if the user making the change is a curator in the group they are changing
