"""
External permissions database models and functions for MyPlatform.
Handles storage of external user groups and their mappings.
Ported from ee/esa/db/external_perm.py
"""
...
from esa.access.utils import build_ext_group_name_for_esa
from esa.configs.constants import DocumentSource
from esa.db.models import PublicExternalUserGroup
from esa.db.models import User
from esa.db.models import User__ExternalUserGroupId
...
class ExternalUserGroup(BaseModel):
    """Represents an external user group from a third-party source."""
    id: str
...
def delete_user__ext_group_for_user__no_commit(
    ...
    """Delete all external group mappings for a specific user."""
    db_session.execute(
...
def delete_user__ext_group_for_cc_pair__no_commit(
    ...
    """Delete all external group mappings for a specific CC pair."""
    db_session.execute(
        delete(User__ExternalUserGroupId).where(
...
def delete_public_external_group_for_cc_pair__no_commit(
    ...
    """Delete all public external groups for a specific CC pair."""
    db_session.execute(
...
def mark_old_external_groups_as_stale(
    ...
    """Mark all external groups for a CC pair as stale before sync."""
    db_session.execute(
...
def upsert_external_user_group(
    db_session: Session,
    external_group: ExternalUserGroup,
    cc_pair_id: int | None = None,
    source: DocumentSource | None = None,
) -> None:
    """
    Upsert a single external user group.
    
    Args:
        db_session: Database session
        external_group: The external group to upsert
        cc_pair_id: Optional CC pair ID
        source: Optional document source
    """
    if cc_pair_id is None or source is None:
        logger.warning("cc_pair_id and source are required for upsert")
        return
    
    upsert_external_groups(
        db_session=db_session,
        cc_pair_id=cc_pair_id,
        external_groups=[external_group],
        source=source,
    )
...
def upsert_external_groups(
    ...
    Args:
        ...
        external_groups: List of external groups to upsert
    ...
    for external_group in external_groups:
        external_group_id = build_ext_group_name_for_esa(
...
def remove_stale_external_groups(
    ...
    """Remove all stale external groups for a CC pair."""
    db_session.execute(
        delete(User__ExternalUserGroupId).where(
            User__ExternalUserGroupId.cc_pair_id == cc_pair_id,
...
def fetch_external_groups_for_user(
    ...
    """Fetch all external group mappings for a user."""
...
def fetch_external_groups_for_user_email_and_group_ids(
    ...
    """Fetch external groups for a user email matching specific group IDs."""
...
def fetch_public_external_group_ids(
    ...
    """Fetch all public external group IDs."""
    return list(
