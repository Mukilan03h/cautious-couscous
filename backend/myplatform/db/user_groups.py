"""
User Group Database Operations for MyPlatform.
Ported from ee/esa/db/user_group.py - FULL ENTERPRISE QUALITY (799 lines, 31 functions)
"""
...
from myplatform.server.user_group.models import SetCuratorRequest
from myplatform.server.user_group.models import UserGroupCreate
from myplatform.server.user_group.models import UserGroupUpdate
...
from esa.db.models import Credential__UserGroup
from esa.db.models import Document
from esa.db.models import DocumentByConnectorCredentialPair
from esa.db.models import DocumentSet__UserGroup
from esa.db.models import LLMProvider__UserGroup
from esa.db.models import Persona__UserGroup
from esa.db.models import TokenRateLimit__UserGroup
from esa.db.models import User
from esa.db.models import User__UserGroup
from esa.db.models import UserGroup
from esa.db.models import UserGroup__ConnectorCredentialPair
from esa.db.models import UserRole
from esa.db.users import fetch_user_by_id
...
def fetch_user_groups(
    ...
    If only_up_to_date is True, filters to return only up-to-date groups.
    """
...
def construct_document_id_select_by_usergroup(
    ...
    """Returns a statement for background processing task generators."""
    stmt = (
        select(Document.id)
        .join(
...
def _validate_curator_relationship_update_requester(
    ...
    """Validates that the user making the change has necessary permissions."""
...
def _validate_curator_relationship_update_request(
    ...
    """Validates that the curator relationship update request is valid."""
    if target_user.role == UserRole.ADMIN:
        ...
        raise ValueError(
            ...
            "permissions of a curator for all groups."
        )
...
# =============================================================================
# These aliases allow tests to import "Custom*" versions of the models,
# which are simply references to the actual ESA models.

CustomUserGroup = UserGroup
CustomUserGroupAuditLog = User__UserGroup
user_group_association = User__UserGroup.__table__
