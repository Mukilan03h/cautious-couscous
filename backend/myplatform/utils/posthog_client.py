"""
PostHog client for MyPlatform.
Ported from ee/esa/utils/posthog_client.py
"""
import json
...
from myplatform.configs.app_configs import MARKETING_POSTHOG_API_KEY
from myplatform.configs.app_configs import POSTHOG_API_KEY
from myplatform.configs.app_configs import POSTHOG_HOST
...
# Marketing PostHog for cross-referencing between cloud and www sites
...
def capture_and_sync_with_alternate_posthog(
    ...
    try:
        if cloud_user_id := props.get("myplatform_cloud_user_id"):
            cloud_props = props.copy()
            cloud_props.pop("myplatform_cloud_user_id", None)
...
def get_marketing_posthog_cookie_name() -> str | None:
    if not MARKETING_POSTHOG_API_KEY:
        return None
    return f"myplatform_custom_ph_{MARKETING_POSTHOG_API_KEY}_posthog"
...
def capture_event(
    ...
    """
    Capture and send an event to PostHog.
...
def flush() -> None:
    ...
    try:
        ...
        logger.error(f"Error flushing PostHog: {e}")
