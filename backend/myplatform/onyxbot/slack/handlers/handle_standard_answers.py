"""
Standard answers handler for Slack bot.
Ported from ee/esa/esabot/slack/handlers/handle_standard_answers.py
"""
...
from esa.db.chat import get_chat_messages_by_sessions
from esa.db.chat import get_chat_sessions_by_slack_thread_id
from esa.db.chat import get_or_create_root_message
from esa.db.models import SlackChannelConfig
from esa.db.models import StandardAnswer as StandardAnswerModel
from esa.esabot.slack.blocks import get_restate_blocks
...
from esa.server.manage.models import StandardAnswer as PydanticStandardAnswer
...
def build_standard_answer_blocks(answer_message: str) -> list[Block]:
    """Build Slack blocks for a standard answer."""
    ...
    return [
        answer_block,
        ActionsBlock(elements=[generate_button_block]),
    ]
...
def _handle_standard_answers(
    ...
    """
    Potentially respond to the user message depending on whether the user's message 
    matches any of the configured standard answers and also whether those answers 
    have already been provided in the current thread.
    
    Returns True if standard answers are found to match the user's message.
    """
    ...
    if matching_standard_answers:
        ...
        answer_blocks = build_standard_answer_blocks(answer_message=answer_message)
