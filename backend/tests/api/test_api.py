...
from esa.main import fetch_versioned_implementation
...
def client() -> Generator[TestClient, Any, None]:
    ...
    app: FastAPI = fetch_versioned_implementation(
        module="esa.main", attribute="get_application"
    )()
...
def test_versions_endpoint(client: TestClient) -> None:
    ...
    assert STABLE_VERSION_PATTERN.match(
        stable["esa"]
    ), f"Stable version {stable['esa']} doesn't match pattern v(number).(number).(number)"
    ...
    assert DEV_VERSION_PATTERN.match(
        dev["esa"]
    ), f"Dev version {dev['esa']} doesn't match pattern v(number).(number).(number)-beta.(number)"

    # Verify migration configuration
    migration = data["migration"]
    assert "esa" in migration
    assert "relational_db" in migration
    assert "index" in migration
    assert "nginx" in migration

    # Verify migration has expected values
    assert migration["esa"] == "airgapped-intfloat-nomic-migration"
    ...
    assert stable["esa"] != dev["esa"], "Stable and dev versions should be different"
