...
from esa.server.documents.models import DocumentSource
...
def test_slack_permission_sync(
    ...
    esa_doc_message_strings = DocumentSearchManager.search_documents(
        query="favorite number",
        user_performing_action=admin_user,
    )
    print(
        "\n documents retrieved by admin user: ",
        esa_doc_message_strings,
    )

    # Ensure admin user can see messages from both channels
    assert public_message in esa_doc_message_strings
    assert private_message in esa_doc_message_strings
    ...
    assert public_message in esa_doc_message_strings
    assert private_message not in esa_doc_message_strings
...
def test_slack_group_permission_sync(
    ...
    """
    This test ensures that permission sync overrides esa group access.
    """
    public_channel, private_channel = slack_test_setup
