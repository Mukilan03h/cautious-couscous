...
Then, using the VSCode/Cursor debugger, run the `ESA OpenAPI Schema Generator` task (see `CONTRIBUTING_VSCODE.md` for `launch.json` setup instructions).
...
```sh
openapi-generator generate -i backend/generated/openapi.json -g python -o backend/generated/esa_openapi_client --package-name esa_openapi_client --skip-validate-spec --openapi-normalizer "SIMPLIFY_ONEOF_ANYOF=true,SET_OAS3_NULLABLE=true"
```

1. Launch esa (using Docker or running with a debugger), ensuring the API server is running on port 8080.
   - If you'd like to set environment variables, you can do so by creating a `.env` file in the esa/backend/tests/integration/ directory.
   - ESA MUST be launched with AUTH_TYPE=basic and ENABLE_PAID_ENTERPRISE_EDITION_FEATURES=true
2. Navigate to `esa/backend`.
...
esa services with a name different from the default `esa`.
