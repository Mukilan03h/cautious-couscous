...
from ee.esa.external_permissions.confluence.doc_sync import confluence_doc_sync
...
@patch("ee.esa.external_permissions.confluence.doc_sync.ESADBCredentialsProvider")
@patch(
    "esa.file_processing.extract_file_text.get_unstructured_api_key",
    return_value=None,
)
def test_confluence_connector_restriction_handling(
    ...
    # this prevents redis calls inside of ESAConfluence
    ...
    non_restricted_emails = {"chris@esa.app"}
    ...
    restricted_emails = {"chris@esa.app", "hagen@danswer.ai", "oauth@esa.app"}
    restricted_user_groups = {"confluence-admins-danswerai"}

    extra_restricted_emails = {"chris@esa.app", "oauth@esa.app"}
    extra_restricted_user_groups: set[str] = set()

    # note that this is only allowed since yuhong@esa.app is a member of the
    # confluence-admins-danswerai group
    special_restricted_emails = {"chris@esa.app", "yuhong@esa.app", "oauth@esa.app"}
