...
@patch("esa.connectors.file.connector.get_default_file_store")
@patch(
    "esa.file_processing.extract_file_text.get_unstructured_api_key", return_value=None
)
def test_single_text_file_with_metadata(
    ...
    file_content = io.BytesIO(
        b'#ONYX_METADATA={"link": "https://esa.app", "file_display_name":"my display name", "tag_of_your_choice": "test-tag", \
          "primary_owners": ["wenxi@esa.app"], "secondary_owners": ["founders@esa.app"], \
    ...
    with patch(
        "esa.connectors.file.connector.get_default_file_store",
    ...
    assert doc.sections[0].link == "https://esa.app"
    assert doc.semantic_identifier == "my display name"
    assert doc.primary_owners[0].display_name == "wenxi@esa.app"  # type: ignore
    assert doc.secondary_owners[0].display_name == "founders@esa.app"  # type: ignore
...
def test_two_text_files_with_zip_metadata(
    ...
    zip_metadata = {
        "file1.txt": {
            ...
            "link": "https://esa.app/1",
            "primary_owners": ["alice@esa.app"],
            "secondary_owners": ["bob@esa.app"],
            "doc_updated_at": "2022-02-02T00:00:00Z",
        },
        "file2.txt": {
            "filename": "file2.txt",
            "file_display_name": "display 2",
            "link": "https://esa.app/2",
            "primary_owners": ["carol@esa.app"],
            "secondary_owners": ["dave@esa.app"],
    ...
    assert doc1.sections[0].link == "https://esa.app/1"
    assert doc1.semantic_identifier == "display 1"
    assert doc1.primary_owners[0].display_name == "alice@esa.app"  # type: ignore
    assert doc1.secondary_owners[0].display_name == "bob@esa.app"  # type: ignore
    assert doc1.doc_updated_at == datetime(2022, 2, 2, 0, 0, 0, tzinfo=timezone.utc)
    assert doc2.sections[0].text == "File 2 content"
    assert doc2.sections[0].link == "https://esa.app/2"
    assert doc2.semantic_identifier == "display 2"
    assert doc2.primary_owners[0].display_name == "carol@esa.app"  # type: ignore
    assert doc2.secondary_owners[0].display_name == "dave@esa.app"  # type: ignore
