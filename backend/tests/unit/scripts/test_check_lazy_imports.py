...
def test_litellm_singleton_eager_import_detection() -> None:
    ...
    test_content = """
import os
from esa.llm.litellm_singleton import litellm  # Should be flagged as eager import
from typing import Dict
...
"""
    ...
    try:
        protected_modules = {"esa.llm.litellm_singleton"}
        result = find_eager_imports(test_path, protected_modules)

        # Should find one violation (line 3)
        assert len(result.violation_lines) == 1
        assert result.violated_modules == {"esa.llm.litellm_singleton"}

        line_num, line = result.violation_lines[0]
        assert "from esa.llm.litellm_singleton import litellm" in line
