...
from esa.connectors.github.connector import GithubConnector
from esa.connectors.github.connector import GithubConnectorStage
...
def test_load_from_checkpoint_with_rate_limit(
    ...
    with patch.object(SerializedRepository, "to_Repository", return_value=mock_repo):
        ...
        end_time = time.time()
        with patch(
            "esa.connectors.github.connector.sleep_after_rate_limit_exception"
        ) as mock_sleep:
            outputs = load_everything_from_checkpoint_connector(
                github_connector, 0, end_time
            )
