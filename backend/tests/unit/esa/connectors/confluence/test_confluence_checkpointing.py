...
from esa.connectors.confluence.connector import ConfluenceCheckpoint
from esa.connectors.confluence.connector import ConfluenceConnector
...
from tests.unit.esa.connectors.utils import load_everything_from_checkpoint_connector
from tests.unit.esa.connectors.utils import (
...
def confluence_connector(
    confluence_base_url: str, space_key: str, mock_confluence_client: ESAConfluence
    ...
    with patch("esa.connectors.confluence.connector._SLIM_DOC_BATCH_SIZE", 2):
        yield connector
...
def test_load_from_checkpoint_with_page_processing_error(
    ...
    with patch(
        "esa.connectors.confluence.connector.ConfluenceConnector._convert_page_to_document",
...
def test_validate_connector_settings_errors(
    ...
    with patch(
        "esa.connectors.confluence.esa_confluence.ESAConfluence.retrieve_confluence_spaces"
    ) as mock_retrieve:
