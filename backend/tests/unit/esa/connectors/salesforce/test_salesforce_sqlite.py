...
from esa.connectors.salesforce.doc_conversion import _extract_section
...
from esa.connectors.salesforce.salesforce_calls import _bulk_retrieve_from_salesforce
from esa.connectors.salesforce.salesforce_calls import _make_time_filter_for_sf_type
from esa.connectors.salesforce.salesforce_calls import _make_time_filtered_query
from esa.connectors.salesforce.salesforce_calls import get_object_by_id_query
...
# from esa.connectors.salesforce.esa_salesforce_type import ESASalesforceType
# from esa.connectors.salesforce.salesforce_calls import get_children_of_sf_type
...
def _create_csv_with_example_data(sf_db: ESASalesforceSQLite) -> None:
    """
...
def _test_query(sf_db: ESASalesforceSQLite) -> None:
    """
...
def _test_upsert(sf_db: ESASalesforceSQLite) -> None:
    """
...
def _test_relationships(sf_db: ESASalesforceSQLite) -> None:
    """
...
def _test_account_with_children(sf_db: ESASalesforceSQLite) -> None:
    """
...
def _test_relationship_updates(sf_db: ESASalesforceSQLite) -> None:
    """
...
def _test_get_affected_parent_ids(sf_db: ESASalesforceSQLite) -> None:
    """
...
def test_salesforce_sqlite() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        sf_db = ESASalesforceSQLite(filename)
        sf_db.connect()
        sf_db.apply_schema()
...
def test_salesforce_bulk_retrieve() -> None:
    ...
    # esa_sf_type = ESASalesforceType("Contact", sf_client)
...
def test_normalize_record() -> None:
    ...
    with open(current_dir / "test_account.csv", "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            normalized_record, parent_ids = ESASalesforceSQLite.normalize_record(row)
...
def test_salesforce_connector_single() -> None:
    ...
    # esa_parent_sf_type = ESASalesforceType(parent_type, sf_client)
    ...
    normalized_record, _ = ESASalesforceSQLite.normalize_record(record)
