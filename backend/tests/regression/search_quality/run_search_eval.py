...
# add esa/backend to path (since this isn't done automatically when running as a script)
current_dir = Path(__file__).parent
esa_dir = current_dir.parent.parent.parent.parent
sys.path.append(str(esa_dir / "backend"))

# load env before app_config loads (since env doesn't get loaded when running as a script)
env_path = esa_dir / ".vscode" / ".env"
...
class SearchAnswerAnalyzer:
    ...
    def _perform_oneshot_qa(self, query: str) -> OneshotQAResult:
        """Perform a OneShot QA query against the ESA API and time it."""
...
def run_search_eval(
    ...
    # check esa api key is set if auth is enabled
    ...
    try:
        ...
        raise RuntimeError(f"Could not connect to ESA API: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Base URL of the ESA API server (default: %(default)s).",
    )
    parser.add_argument(
        "-s",
        "--search_only",
