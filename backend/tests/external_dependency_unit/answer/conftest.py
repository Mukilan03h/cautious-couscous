...
def mock_nlp_embeddings_post() -> Iterator[None]:
    ...
    with patch(
        "esa.natural_language_processing.search_nlp_models.requests.post",
...
def mock_gpu_status() -> Iterator[None]:
    ...
    with patch(
        "esa.utils.gpu_utils._get_gpu_status_from_model_server", return_value=False
    ):
        yield
...
def mock_vespa_query() -> Iterator[None]:
    ...
    with patch("esa.document_index.vespa.index.query_vespa", return_value=[]):
        yield
