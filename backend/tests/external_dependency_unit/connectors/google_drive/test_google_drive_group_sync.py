...
from ee.esa.background.celery.tasks.external_group_syncing.tasks import (
    _perform_external_group_sync,
)
from ee.esa.db.external_perm import ExternalUserGroup
...
class TestPerformExternalGroupSync:
    ...
    def test_initial_group_sync(self, db_session: Session) -> None:
        ...
        with patch(
            "ee.esa.background.celery.tasks.external_group_syncing.tasks.get_source_perm_sync_config"
        ) as mock_config:
            # Mock sync config
            ...
            expected_group1_id = build_ext_group_name_for_esa(
                "group1", DocumentSource.GOOGLE_DRIVE
            )
            expected_group2_id = build_ext_group_name_for_esa(
                "group2", DocumentSource.GOOGLE_DRIVE
            )
            expected_public_group_id = build_ext_group_name_for_esa(
    ...
    def test_mixed_regular_and_public_groups(self, db_session: Session) -> None:
        ...
        with patch(
            ...
            # Verify user groups
            user_groups = _get_user_external_groups(db_session, cc_pair.id)
            expected_regular_group_id = build_ext_group_name_for_esa(
                "regular_group", DocumentSource.GOOGLE_DRIVE
            )
            expected_public_group1_id = build_ext_group_name_for_esa(
            ...
            expected_public_group2_id = build_ext_group_name_for_esa(
