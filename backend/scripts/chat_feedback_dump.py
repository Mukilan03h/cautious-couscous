...
from ee.esa.server.query_history.api import ChatSessionSnapshot
...
def create_new_chat_session(esa_url: str, api_key: str | None) -> int:
    headers = {"Authorization": f"Bearer {api_key}"} if api_key else None
    session_endpoint = esa_url + "/api/chat/create-chat-session"
...
def manage_users(esa_url: str, headers: dict[str, str] | None) -> AllUsersResponse:
    endpoint = esa_url + "/manage/users"
...
def get_chat_sessions(
    esa_url: str, headers: dict[str, str] | None, user_id: UUID
) -> ChatSessionsResponse:
    endpoint = esa_url + "/admin/chat-sessions"
...
def get_session_history(
    esa_url: str, headers: dict[str, str] | None, session_id: UUID
) -> ChatSessionSnapshot:
    endpoint = esa_url + f"/admin/chat-session-history/{session_id}"
...
def process_all_chat_feedback(esa_url: str, api_key: str | None) -> None:
    headers = {"Authorization": f"Bearer {api_key}"} if api_key else None

    all_users = manage_users(esa_url, headers)
    if not all_users:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="ESA URL, should point to ESA nginx.",
    )

    # Not needed if Auth is disabled?
    # Or for ESA MIT Edition API key must be replaced with session cookie
    parser.add_argument(
        "--api-key",
        type=str,
        help="ESA Admin Level API key",
    )

    args = parser.parse_args()
    process_all_chat_feedback(esa_url=args.url, api_key=args.api_key)
