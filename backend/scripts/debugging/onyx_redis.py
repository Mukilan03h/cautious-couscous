...
from ee.esa.server.tenants.user_mapping import get_tenant_id_for_email
...
from esa.configs.app_configs import REDIS_DB_NUMBER
from esa.configs.app_configs import REDIS_HOST
from esa.configs.app_configs import REDIS_PASSWORD
from esa.configs.app_configs import REDIS_PORT
from esa.configs.app_configs import REDIS_SSL
...
from esa.redis.redis_pool import RedisPool
...
def esa_redis(
    ...
    if command == ESARedisCommand.purge_connectorsync_taskset:
    ...
    elif command == ESARedisCommand.purge_documentset_taskset:
        return purge_by_match_and_type(
            "*documentset_taskset*", "set", batch, dry_run, r
        )
    elif command == ESARedisCommand.purge_usergroup_taskset:
        return purge_by_match_and_type("*usergroup_taskset*", "set", batch, dry_run, r)
    elif command == ESARedisCommand.purge_locks_blocking_deletion:
        ...
        return 0
    elif command == ESARedisCommand.purge_vespa_syncing:
        return purge_by_match_and_type(
            "*connectorsync:vespa_syncing*", "string", batch, dry_run, r
        )
    elif command == ESARedisCommand.purge_pidbox:
        return purge_by_match_and_type(
            "*reply.celery.pidbox", "list", batch, dry_run, r
        )
    elif command == ESARedisCommand.get_list_element:
        ...
        print(f"{result}")
        return 0
    elif command == ESARedisCommand.get_user_token:
        if not user_email:
        ...
        if token_key:
            ...
            return 2
    elif command == ESARedisCommand.delete_user_token:
        if not user_email:
        ...
        if delete_user_token_from_redis(r, user_email, dry_run):
            ...
            return 2
    elif command == ESARedisCommand.add_invited_user:
        if not user_email:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ESA Redis Manager")
    parser.add_argument(
        "--command",
        type=ESARedisCommand,
        help="The command to run",
        choices=list(ESARedisCommand),
        required=True,
    )
