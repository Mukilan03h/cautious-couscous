...
# In this case, the equivalent of asking a question in ESA Chat in a new chat session
import argparse
import json
import os
...
def process_question(esa_url: str, question: str, api_key: str | None) -> None:
    message_endpoint = esa_url + "/api/chat/send-message"

    chat_session_id = create_new_chat_session(esa_url, api_key)
...
if __name__ == "__main__":
    ...
    # Or for ESA MIT API key must be replaced with session cookie
    api_key = os.environ.get("DANSWER_API_KEY")

    args = parser.parse_args()
    process_question(
        esa_url=args.esa_url, question=args.test_question, api_key=api_key
    )
